...
/** Case à cocher d'un joueur dans le formulaire : absent, a joué, ou a gagné. */
...
@Component({
  ...
  template: `
    ...
    <div class="picker">
      ...
      <div class="tiles">
        @for (g of store.games(); track g.id) {
          <button type="button" class="chip tile" [attr.data-tone]="tone(g.id)"
            [attr.aria-pressed]="formOpen() && gameId() === g.id" (click)="start(g.id)">
            <span class="tile-name">{{ g.name }}</span>
            <span class="tile-meta">{{ range(g) }} joueurs · {{ plays().get(g.id) ?? 0 }} partie{{ (plays().get(g.id) ?? 0) > 1 ? 's' : '' }}</span>
    ...
    @if (formOpen()) {
      <form class="form" [attr.aria-label]="'Nouvelle partie de ' + selectedGame()?.name" (submit)="$event.preventDefault(); save()">
        <div class="form-row">
          <label>
            <span class="label">Date</span>
            <input class="field" type="date" [value]="playedOn()" (input)="playedOn.set($any($event.target).value)" />
          </label>
          <label class="grow">
            <span class="label">Note</span>
            <input class="field" placeholder="Optionnel : score, anecdote…" [value]="notes()"
          ...
          @for (group of seatGroups(); track group.id) {
            <section class="dept" [attr.data-tone]="group.tone">
              ...
              @for (p of group.items; track p.id) {
                <button type="button" class="seat" [attr.data-state]="seat(p.id)" (click)="cycle(p.id)"
        ...
        <div class="form-foot">
          @if (selectedGame(); as g) {
            <span class="count" [class.warn]="!fits()">
              ...
              @if (!fits()) { · {{ g.name }} se joue à {{ range(g) }} }
            </span>
          }
          <button class="btn-link" type="button" (click)="close()">Annuler</button>
          <button class="btn-lime" type="submit" [disabled]="!canSave() || saving()">✓ Enregistrer la partie</button>
        </div>
      </form>
    }
        ...
        <tbody>
          @for (m of matches(); track m.id) {
            <tr>
              ...
              <td class="game">
                <app-game-chip [gameId]="m.game_id" [name]="store.gamesById().get(m.game_id)?.name ?? '?'" />
              </td>
              <td>
                <div class="people">
              ...
              <td class="actions">
                <button class="btn-icon" type="button" (click)="remove(m)" aria-label="Supprimer la partie">×</button>
              </td>
            </tr>
          }
        </tbody>
  ...
  styles: `
    ...
    .tile { position: relative; flex-direction: column; align-items: flex-start; justify-content: center; gap: 2px;
      height: auto; min-height: 64px; padding: 10px 40px 10px 14px; border-radius: 12px; text-align: left;
      transition: transform 0.12s, box-shadow 0.12s; }
    .tile::before { position: absolute; top: 16px; left: 14px; }
    .tile-name { padding-left: 14px; font-size: 13px; font-weight: 600; }
    .tile-meta { font-size: 11px; letter-spacing: 0; text-transform: none; opacity: 0.75; }
    .tile-plus { position: absolute; right: 12px; top: 50%; translate: 0 -50%; display: grid; place-items: center;
      width: 24px; height: 24px; border-radius: 50%; background: #fff; font-size: 14px; }
    .tile:hover { transform: translateY(-1px); box-shadow: 0 4px 12px rgb(0 0 0 / 0.06); }
    .tile[aria-pressed='true'] { border-color: var(--chip-dot); box-shadow: 0 0 0 2px var(--chip-dot); }
    .tile[aria-pressed='true'] .tile-plus { background: var(--chip-dot); color: #fff; }
    .count { margin-right: auto; font-size: 13px; color: #55555c; }
    .count.warn { color: var(--danger); }
    .date { width: 120px; white-space: nowrap; text-transform: capitalize; }
    .game { width: 1%; }
    .people { display: flex; flex-wrap: wrap; gap: 6px 14px; }
    .person { color: #55555c; }
    .winner { color: var(--ink); font-weight: 600; }
    .notes { margin-top: 2px; font-size: 12px; color: var(--muted); }
    .form { margin: 12px 0 8px; padding: 16px; border-radius: 12px; background: #f8f7f4; border: 1px solid var(--line); }
    ...
    .label { display: block; margin-bottom: 6px; font-size: 11px; font-weight: 600; letter-spacing: 0.06em;
      text-transform: uppercase; color: #55555c; }
    .board { display: grid; grid-template-columns: repeat(auto-fill, minmax(190px, 1fr)); gap: 10px; }
    .dept { display: flex; flex-direction: column; gap: 6px; padding: 10px; border: 1px solid var(--line);
      border-radius: 12px; background: var(--panel); }
    .dept-head { display: flex; align-items: center; justify-content: space-between; margin-bottom: 4px; }
    .dept-count { font-size: 12px; font-weight: 600; color: var(--muted); font-variant-numeric: tabular-nums; }
    .seat { display: flex; align-items: center; gap: 8px; min-height: 34px; padding: 4px 10px; text-align: left;
      border: 1px solid var(--line); border-radius: 8px; background: #fff; font-size: 13px; }
    .seat:hover { border-color: var(--accent); }
    .seat-mark { display: inline-grid; place-items: center; width: 18px; height: 18px; flex: none;
      border: 1.5px solid #d8d7d2; border-radius: 5px; font-size: 11px; }
    .seat[data-state='played'] .seat-mark { border-color: var(--accent); background: var(--accent); color: #fff; }
    .seat[data-state='won'] .seat-mark { border: 0; }
    .seat-name { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .seat[data-state='played'] { background: var(--accent-soft); border-color: var(--accent); color: var(--accent-ink); font-weight: 500; }
    .seat[data-state='won'] { background: var(--lime); border-color: #d9dd62; font-weight: 600; }
    .form-foot { display: flex; justify-content: flex-end; align-items: center; gap: 16px; margin-top: 16px; }
  `,
})
export class Matches {
  ...
  protected readonly seatLabel: Record<Seat, string> = { out: 'absent', played: 'a joué', won: 'a gagné' };
  ...
  protected readonly selectedGame = computed(() => this.store.gamesById().get(this.gameId()) ?? null);
  protected readonly playerCount = computed(() => Object.values(this.seats()).filter((s) => s !== 'out').length);
  ...
  protected readonly seatGroups = computed(() => groupByDepartment(this.store.players(), (p) => p.department));
  ...
  protected remove(match: Match): void {
    ...
    if (confirm(`Supprimer la partie de ${game} du ${match.played_on} ?`)) this.store.deleteMatch(match.id);
  }
}
